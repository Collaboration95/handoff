...
- [x] Write hand-derived tests first: six billable units at SGD90 gives subtotal 54000 minor units; an old SGD100 contract fails. Wrong customer, PO, terms, currency, quantity, and evidence IDs must fail. Unsigned/future amendments cannot win; conflicting latest contracts and unaccepted work require review.
- [x] Run tests and observe the missing behavior before implementation.
- [x] Implement pure validation and a structured-output model client using the configured model. Preserve objective text exactly. Reject malformed candidates and perform no extra repair when the initial plan passes.
- [x] Add engine tests with a fake model boundary: one invalid and one valid candidate; all-invalid candidates; provider exception; malformed model output; objective preservation; candidate limit.
- [x] Run the core tests and one live model case. Report test command, results, changed files, and any concerns.
...
- [x] Confirm create/add-line-items/retrieve schemas from official docs; record the exact payloads in tests. No endpoint guessing.
- [x] Test the fixed CLI runner boundary, asserting commands, field values, Decimal conversion, and readback verification. Test create success followed by add-item failure and uncertain responses; never claim full success or silently recreate.
- [x] Use official CLI sandbox OAuth, create DRAFT/OUT_OF_BAND, add PER_UNIT pricing, and read back. Persist a journal before each write, with stable operation request IDs and partial invoice IDs. No finalize, pay, or send operation.
- [x] Verify live after authenticated sandbox readiness. Surface permission/IP/Billing errors distinctly without secret-bearing response data.
...
- [x] Trace baseline, checks, candidates, convergence, and external readback with correlated request/case IDs. Add an explicit local-only status if credentials are absent.
- [x] Implement hand-labelled fixtures and independent scoring. A review on a resolvable case is not a successful invoice. Report all cases and separate fault injection from real-model cases.
- [x] Test scores on literal expected plans. Run the suite with real model output and retain actual results, including ties and regressions.
- [x] Verify one actual Langfuse trace and its linked metrics before claiming hosted auditing works.
...
- [x] Write API tests for objective preservation, invalid input, unconfigured provider, and rejecting unverified invoice writes.
- [x] Serve one page showing source records, original and selected invoice, field checks, real external invoice status, trace link, tokens, and latency. Clearly label injected faults, local traces, and recorded results.
- [x] The create endpoint revalidates the plan server-side before Airwallex access. Browser never receives credentials.
- [x] Run all tests, start the server, exercise a real request through the UI, and inspect the Airwallex readback and Langfuse trace when configured.

## Task 5 — Integration and rehearsal

- [x] Fetch teammates' changes and integrate through one owner; never force-push or overwrite shared work.
- [ ] Freeze at minute 100. Rehearse the two-person, 120-second script in finance/DEMO.md. Mark any recorded replay as replay.

...
- **Voice recovery check:** a later test sent verified synthetic speech through LiveKit and received 5,423 nonzero output samples from the agent. A separate direct GPT-Live check also received an audible response. The reconnect supervisor now resets to a one-second delay after normal idle-room closure; repeated transport failures retain capped exponential backoff. These checks verify audio transport, not whether a particular user's browser remained connected or its speakers played the response.
...
- **Client initialization:** the installed React SDK 2.9.20 missed the initial state of an already-listening agent and falsely ended the browser call after 20 seconds. The official 2.9.21 fix, with its required `livekit-client` 2.18.2 peer, reads the existing participant attributes. The updated browser remained connected for over 40 seconds, then left and rejoined successfully for over a minute. A synthetic audio check with that browser present received 6,197 nonzero output samples and the browser displayed Handoff's response. Actual missing-agent and initialization timeouts remain enabled.
...
137 tests passed after adding the dedicated autofix simulation, draft reconciliation, and voice recovery fixes. The checks cover repair without approval or external writes, approval of the displayed correction without a second model call, honest repair failure, read-only verification of existing drafts, mismatch and uncertain-write protection, voice creation timeouts, safe conflict messages, safe audio diagnostics, and prompt reconnection after normal room closure. HTTP transport tests require permission to bind a local test server. The only warning was an upstream Starlette/AnyIO deprecation. Independent review found no blockers in the reconciliation path; earlier findings on proposal races, microphone lifecycle, paired latency, and selected-candidate display were fixed and reviewed again.
...
The client readiness regression suite passes all six cases on React SDK 2.9.21. The same final suite fails three cases on 2.9.20 while preserving three genuine-failure cases. It exercises the actual session and agent hooks, initial participant state, attribute updates, reconnects, and failure timeouts. The upgraded client also passed its production build.
